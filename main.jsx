...
import { getFirestore, collection, onSnapshot, doc, setDoc, query } from 'https://www.gstatic.com/firebasejs/10.12.0/firebase-firestore.js';
        ...
        <form onSubmit={handleSubmit}>
          <div className="space-y-4">
            <div>
              ...
              <select id="host" value={host} onChange={(e) => setHost(e.target.value)} className="w-full p-2 border border-gray-300 rounded-lg focus:ring-teal-500 focus:border-teal-500">
                {hosts.map(h => <option key={h} value={h}>{h}</option>)}
              </select>
            </div>
            <div>
              <label htmlFor="location" className="block text-sm font-medium text-gray-700 mb-1">اختر المكان</label>
              <select id="location" value={location} onChange={(e) => setLocation(e.target.value)} className="w-full p-2 border border-gray-300 rounded-lg focus:ring-teal-500 focus:border-teal-500">
      ...
      <div className="bg-white rounded-xl shadow-2xl p-6 sm:p-8 w-full max-w-lg" dir="rtl">
        ...
        <div className="mt-6 text-center bg-teal-50 p-4 rounded-lg">
          ...
          <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <button onClick={generateInvitation} disabled={isLoading} className="flex items-center justify-center w-full px-4 py-2 bg-white border border-teal-600 text-teal-700 rounded-lg hover:bg-teال-100 disabled:opacity-50 transition">
              ✨ إنشاء رسالة دعوة
            </button>
          </div>
        </div>
...
function App() {
  ...
  const hijriMonths = useMemo(() => [
    { name: 'ربيع الثاني', index: 4 },
    { name: 'جمادى الأولى', index: 5 },
    { name: 'جمادى الآخرة', index: 6 },
    { name: 'رجب', index: 7 },
    { name: 'شعبان', index: 8 },
    { name: 'شوال', index: 10 },
    { name: 'ذو القعدة', index: 11 },
    { name: 'ذو الحجة', index: 12 },
    { name: 'محرم', index: 1 },
    { name: 'صفر', index: 2 },
    { name: 'ربيع الأول', index: 3 }
  ], []);
  ...
  useEffect(() => {
    try {
      ...
      console.error('Error initializing Firebase:', e); setIsLoading(false);
    }
  }, []);
  ...
  useEffect(() => {
    if (isAuthReady && db) {
      ...
      const unsubscribe = onSnapshot(q, (querySnapshot) => {
        const bookingsData = querySnapshot.docs.map(d => ({ id: d.id, ...d.data() }));
    ...
    <div dir="rtl" className="bg-gray-50 min-h-screen font-sans text-gray-800 p-4 sm:p-6 lg:p-8">
      <div className="max-w-4xl mx-auto">
        <header className="text-center mb-8">
          <h1 className="text-3xl sm:text-4xl font-bold text-teال-700">جدول الدورية الشهرية للأسرة</h1>
          <p className="text-gray-500 mt-2">عام {hijriYear} هـ</p>
          <p className="mt-4 text-sm max-w-2xl mx-auto text-gray-600">"مَنْ أَحَبَّ أَنْ يُبْسَطَ لَهُ فِي رِزْقِهِ، وَيُنْسَأَ لَهُ فِي أَثَرِهِ، فَلْيَصِلْ رَحِمَهُ"</p>
        </header>
        <main className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {hijriMonths.map((month) => (
            <MonthCard key={month.name} month={month} booking={getBookingForMonth(month.name)} onBook={handleOpenModal} />
          ))}
        </main>
        <footer className="text-center mt-10 text-sm text-gray-400"><p>نسأل الله أن يديم علينا الوصل والمحبة.</p></footer>
      </div>
      {showModal && (
        <BookingModal month={selectedMonth} hosts={hosts} locations={locations} onConfirmBooking={handleConfirmBooking} onClose={handleCloseModal} initialError={error} />
      )}
      {showSuccessModal && latestBooking && (
        <GeminiSuccessModal booking={latestBooking} onClose={() => setShowSuccessModal(false)} />
      )}
    </div>
  );
}
